...
import { initialValuesRTGSFile, validationSchemaRTGSFile } from './utils';
...
export default function RTGSFile(): ReactNode {
  ...
  const columnDefs = [
    ...
    { field: 'rtgsFileName', headerName: 'RTGS File' },
  ...
  return (
    <Box>
      <Box>
        <Formik initialValues={initialValuesRTGSFile} validationSchema={validationSchemaRTGSFile} onSubmit={handleSubmitATMFile} validateOnBlur={false}>
                    ...
                    <Card className="flex flex-col items-start w-88 h-[140px] p-8 mr-4 rounded-none overflow-auto ">
                      <Box className="flex-column my-8">
                        <Box sx={{ marginTop: '10px', marginLeft: '10px' }}>
                          <FormLabel sx={{ fontWeight: 'bold', color: '#000', fontSize: '14px', marginTop: '10px', marginLeft: '10px' }}>RTGS File</FormLabel>
                        </Box>
                            ...
                            <Box sx={{ marginTop: '10px', marginLeft: '20px' }}>
                              <Button type="basic" component="label" sx={btnClass}>
                                ...
                                <input
                                  ...
                                  accept=".xlsx"
                                  onChange={(event) => {
                                    handleOnChange(event, 'rtgsFile', setFile, setFieldValue);
                                  }}
                                />
                              </Button>
                            </Box>
                        ...
                        <Box className="mt-6 flex-row" sx={{ marginTop: '10px', marginLeft: '20px' }}>
                          <span className="text-xs break-words max-w-[300px] inline-block">
                            {values?.rtgsFile?.name}
                            {values?.rtgsFile && (
                              <CloseIcon
                                onClick={() => {
                                  setFieldValue('rtgsFile', undefined);
                                  setFile(undefined);
                                }}
                    ...
                    <Box className="flex justify-end mt-20 px-4">
                      <Box>
                        <Button
                          ...
                          sx={{ marginTop: '20px' }}
                          disabled={values?.rtgsFile === undefined || values?.rtgsFile === null}
                        >
                          Reconcile
                        </Button>
                      </Box>
